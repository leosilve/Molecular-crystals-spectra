...
//	cout << "Computing crystal emission in vibronic approximation ... " << endl;
	...
	int i, j, m, mode, state, dwtot, hitot, mol, vib_pos;
...
// This is the routine for 2 modes and only one particle states	
	if (get_NMODES()==2)
	{
		...
		for (dwtot=0; dwtot<=maxdw; dwtot++)	for (hitot=0; hitot<=maxhi; hitot++)
		{
...
// For the moment it only gives the 0-0 and the 0-1	
	else  
	{

// The limits in the for loops here are manually set because we are only considering 0-0 and 0-1 for a single mode		
		for (dwtot=0; dwtot<=1; dwtot++)	for (hitot=0; hitot<=0; hitot++)
		{
//			cout << dwtot << hitot << endl;
			FinVibs[0]=dwtot;
			FinVibs[1]=hitot;
			
//--------- 0-0 emission is the same as for NP==1
			if (dwtot+hitot==0)
			{				
				if (norm_2(get_KVEC())<ZERO_TOL || norm_2(get_KVEC()-get_latticePtr()->get_KLIGHT())<ZERO_TOL)
				{
				...
				for (mol=0; mol<(*latticePtr).get_N_MOL(); mol++)
				{
					...
					for (state=0; state<emitting_state.size(); state++)
					{
						...
						if ( BASIS[state].get_np()==2 )
						{
							...
							vib_pos=0;
							temp_dipole[0]=0.0; temp_dipole[1]=0.0;temp_dipole[2]=0.0;
							for (m=1; m<BASIS[state].get_max_cloud(); m++)
								if (BASIS[state].get_cloud(mode,m)>0)
									vib_pos=m;	
							if (vib_pos==0) nrerror("compute_crystal_emission: could not find vib_pos in a two-particle state");
							if (BASIS[state].get_cloud(mode,m)==1 && get_CLOUD_NN_alpha(BASIS[state].get_mol(),vib_pos)==mol)
							{
								for (i=0; i<3; i++)
									temp_dipole(i)=(*latticePtr).get_MOL_DIPOLE(BASIS[state].get_mol())(i)*emitting_state.get_coeff(state)
									*exp(-I*inner_prod(get_KVEC(),(*latticePtr).get_RHO(mol)))
...
//	else
//		nrerror("compute_crystal_emission: it works for (NP==1 and NMODES<=2) or for (NP==2 and NMODES==1)");
	return(emission);
}
/*

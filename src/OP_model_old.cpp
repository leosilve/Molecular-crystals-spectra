...
//		nrerror("OPmodel<OPFLOAT,basis>::compute_emission: grid has an odd number of points!");
	...
	dwmax=basisPtr->get_MAX_VIB(0)+1;
	if (basisPtr->get_NMODES()>1)
		himax=basisPtr->get_MAX_VIB(1)+1;
	else
		himax=1;
	...
	{
//		if (n0%2==0)
//			ka=basisPtr->get_latticePtr()->get_KCELL(0)*(-n0/2+1+i0)/n0;
//		else
			ka=basisPtr->get_latticePtr()->get_KCELL(0)*(i0-(n0-2+n0%2)/2)/n0;
		for (i1=0; i1<n1; i1++)
		{
//			if (n1%2==0)
//				kb=(*(*basisPtr).get_latticePtr()).get_KCELL(1)*(-n1/2+1+i1)/n1;
//			else
				kb=basisPtr->get_latticePtr()->get_KCELL(1)*(i1-(n1-2+n1%2)/2)/n1;
			for (i2=0; i2<n2; i2++)
			{
//				if (n2%2==0)				
//					kc=(*(*basisPtr).get_latticePtr()).get_KCELL(2)*(-n2/2+1+i2)/n2;
//				else
	...
	infile >> dwmax;
	infile >> himax;
	state_emission.resize(dwmax);
	for (dwtot=0; dwtot<dwmax; dwtot++)
		state_emission[dwtot].resize(himax);

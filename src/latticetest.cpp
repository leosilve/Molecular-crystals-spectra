...
#include <math.h>
#include <vector>
/*#include <My_boost_N_Functions.h>
#include <physics.h>*/
#include <lattice.h>
					    ...
					    LATFLOAT rmax, int flag_macro, ComplexMatrix &Ltilde, ComplexMatrix &udc, vector<LATFLOAT> &Values)
{
	...
	switch (MODE[0]) 
	{
				case 'F':
					...
					for (i=0; i<N_MOL; i++)	for (j=i; j<N_MOL; j++) udc(i,j)=Ltilde(i,j);
					break;
				case 'S':
					flag_2D=1;
					Finite_sums(flag_2D, mol_mol_int, NON_SCR_RADIUS, NN_int, rmax, kvec, Ltilde);
...
// Completes the Ltilde Hermitian matrix (not needed for udc because we use uplo='U' in routine heev
	for (i=0; i<N_MOL; i++)
		for (j=i+1; j<N_MOL; j++)
			Ltilde(j,i)=conj(Ltilde(i,j));
...
template<typename LATFLOAT>	
void lattice<LATFLOAT>::compute_FE_bands(vector<Vector > &dir, int npoints, string MODE, 
					  string mol_mol_int, LATFLOAT NON_SCR_RADIUS, vector<vector<LATFLOAT> >& NN_int,
					  LATFLOAT rmax, int flag_macro, int n0, int n1, int n2, Vector& KLIGHT)
{
...
// Here we compute energies, eigenvectors and dipoles at k=KLIGHT
	...
	for (dc=0; dc<MyValues.size(); dc++)
	{
		...
		for (i=0; i<3; i++)	dipole(i)=0.0;
		for (i=0; i<MyValues.size(); i++)
				dipole+=udc(i,dc)*exp(-I*inner_prod(k,RHO[i]))*MOL_DIPOLE[i];
